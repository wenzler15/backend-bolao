...
export class UsersService {
  ...
  async forgotPassword(email: string) {
    if (!email) return { message: 'Please send a e-mail' };
  ...
  async checkPayment(body: any) {
    ...
    if (body.gameMode === 1) {
      ...
      if (!status) return { message: 'Payment not found!' };
  ...
  async historyBets(id: string) {
    const betsLeftOne = await this.betOneLeftRepository
      ...
      .select([
        'betOneLeft.userId, betOneLeft.life, betOneLeft.matchId, betOneLeft.round, betOneLeft.leagueId, betOneLeft.status , awayTeam.teamEmblemUrl AS "awayTeamEmblemUrl", awayTeam.teamName AS "awayTeamName", homeTeam.teamEmblemUrl AS "homeTeamEmblemUrl", homeTeam.teamName AS "homeTeamName", winnerTeam.teamName as "winnerTeam"',
      ])
    ...
    const betsRound = await this.betRoundRepository
      ...
      .innerJoinAndSelect(
        'team',
        'winnerTeam',
        'winnerTeam.teamId = betRound.winnerTeam',
      )
      ...
      .select([
        'betRound.userId, betRound.matchId, betRound.round, betRound.homeTeamScore, betRound.awayTeamScore , awayTeam.teamEmblemUrl AS "awayTeamEmblemUrl", awayTeam.teamName AS "awayTeamName", homeTeam.teamEmblemUrl AS "homeTeamEmblemUrl", homeTeam.teamName AS "homeTeamName", winnerTeam.teamName as "winnerTeam"',
      ])
    ...
    if (round) {
      ...
      response = await this.betOneLeftRepository
        ...
        .innerJoinAndSelect(
          'round',
          'round',
          'round.matchId = betLeftOne.matchId',
        )
        .innerJoinAndSelect(
          'team',
          'team',
          'team.teamId = betLeftOne.winnerTeamId',
        )
        .select([
          'betLeftOne.userId, betLeftOne.life, betLeftOne.matchId, user.name, user.favoriteTeam, round.leagueId, betLeftOne.wins, betLeftOne.draws, betLeftOne.awayTeamBet, team.teamEmblemUrl',
        ])
        .orderBy('betLeftOne.life', 'ASC')
        .getRawMany();
      ...
      response.forEach(async (item, index) => {
        if (item.leagueId == league) {
          if (item.userId == id && userRanking.length == 0) {
            userRanking.push(item);
          }
  ...
  async resetPassword(body: ResetPasswordEntity) {
    ...
    if (token !== user.passwordResetToken) return { message: 'Wrong token' };
